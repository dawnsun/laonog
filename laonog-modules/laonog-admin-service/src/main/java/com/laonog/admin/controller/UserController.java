...
public class UserController extends BaseController {
    ...
    public R<Boolean> userDel(@PathVariable Long id) {
        SysUserDO sysUserDO = userService.deleteUserById(id);
        return new R<>(userService.deleteUserById(sysUserDO));
    }
    ...
    public R<Boolean> user(@RequestBody UserDTO userDto) {
        ...
        sysUserDO.setDelFlag(CommonConstant.STATUS_NORMAL);
        sysUserDO.setPassword(ENCODER.encode(userDto.getNewpassword1()));
        userService.insert(sysUserDO);

        userDto.getRole().forEach(roleId -> {
            SysUserRole userRole = new SysUserRole();
            userRole.setUserId(sysUserDO.getUserId());
    ...
    public R<Boolean> userUpdate(@RequestBody UserDTO userDto) {
        SysUserDO user = userService.selectById(userDto.getUserId());
